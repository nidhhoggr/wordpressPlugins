    ...
    public function init($settings = array())
    {
        ...
        if($this->hasHooks) {
            ...
            foreach($hook_types as $hook_type) {
                ...
                $hookNamespacedClassName = '\SupraCsvPremium\\' . $hookClassName;
    ...
    public function ingestContent($mapping = array()) 
    {
        ...
        if($cols) {
            foreach($this->csvLines as $data) {
                ...
                $rp_result = $this->remotePost->injectListing($post_args);

                if($rp_result && !($rp_result instanceof \SupraCsvPremium\XMLRPC_Error))
                {
                ...
                else if($rp_result instanceof \SupraCsvPremium\XMLRPC_Error)
                {
                    $this->progressBuffer .= $this->rowIngestionFailure($post_args, $rowCount . ' with an issue: ' . $rp_result);
                }
                else
                {
...
class SupraCsvMapperForm {
    ...
    public function getForm() {
        ...
        $inputs .= '<h3>Attachments</h3>';
        $inputs .= self::createInput('attachments','Attachments',$this->rows);
