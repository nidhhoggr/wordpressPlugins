...
class SupraCsvExtractor extends ExtractorArgumentParser {
    ...
    **/   
    ...
    private function getCustomFields() {
        ...
        $this->postinfo['custom_fields'] = array();
...
class SupraCsvExporter extends ExporterArgumentParser {
    ...
    private function parseRecords() {
        ...
        foreach((array)$this->posts as $i=>$post) 
        {
            $i = $i+1;
            ...
            foreach($this->parsable_keys as $key=>$pk) 
            {
                ...
                {
                    $this->records[$i][$pk] = $post['post']->$pk;
                }
                ...
                { 
                    foreach((array)$pk as $p) {
                        ...
                        {
                            $this->records[$i][$p] = $post['postinfo'][$key][$p];
                        }
                        else 
                        {
                            $this->records[$i][$p] = null;
                        }
                    }
                }
...
/**
        ...
        //var_dump($this->csvstring);
**/
        return $this;
    }

    public function download($export_settings) {
        if(empty($this->csvstring)) $this->parseRecords()->buildCsv($export_settings);

        return $this->csvstring;
    }

    public function createFile($export_settings) {
        if(empty($this->csvstring)) $this->parseRecords()->buildCsv($export_settings);
