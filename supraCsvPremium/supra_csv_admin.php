...
$scp = new \SupraCsvPremium\SupraCsvPlugin();
...
<form name="scsv_form" method="post">
        ...
        <h3>Performance Options</h3>
          ...
          <p>
            ...
            Chunk Ingestion<input type="checkbox" name="scsv_is_ingestion_chunked" value="1" />
          </p>

          <script>

            var supra_csv_settings = {
                'is_ingestion_chunked': <?php echo ((int) $misc_options['is_ingestion_chunked']) ?>
            }

            $(function() {

                if(supra_csv_settings.is_ingestion_chunked)
                {
                    $('input[name=scsv_is_ingestion_chunked]').click();
                    $('#chunk_ingestion_options').show();
                }
            });
            
            $('input[name=scsv_is_ingestion_chunked]').on('change', function() {
        ...
        <p id="chunk_ingestion_options">
            ...
            <input name="scsv_chunk_by_n_rows" value="<?=$misc_options['chunk_by_n_rows']?>" size="4" style="display: inline; float: none" maxlength="4"> rows

            <br />
            <span id="isusingmultithreads_tt" class="tooltip"></span>
            Use Multi-Threading<input type="checkbox" name="scsv_is_using_multithreads" value="1" <? echo ($misc_options['is_using_multithreads'])?'checked="checked"':'';?>/
        </p>
...
<div style="float: right; width: 300px;">
        ...
        <p id="custom_terms">
          <span id="customterms_tt" class="tooltip"></span>Custom Terms (<span style="color: red">separate terms with commas</span>)
            <input type="text" name="scsv_custom_terms" value="<?php echo $post_terms?>" id="scsv_custom_terms">
        </p>
