...
-->
        ...
        <p>
            <span id="customposttype_tt" class="tooltip"></span>Custom Post Type
            <input type="text" name="sscrap_custom_posttype" value="" size="5">
        </p>
...
<!-- /***pps***/ -->
        ...
        <h3>Meta Key Mapping<span id="metakeymapping_tt" class="tooltip"></span></h3>
...
<?php
    $metakeymapping = $uc->getMetaKeyMapping();

    foreach($metakeymapping as $i=>$mm_item)
    {
        echo '<tr id="pm_info'.$i.'" class="pm_info">';

        foreach($mm_item as $mm_col_k=>$mm_col_v)
        {
            switch($mm_col_k)
            {
                case "meta_key":
                    echo '<td><input type="text" name="sscrap_meta_key[]" id="meta_key" value="'.$mm_col_v.'"></td>';
                    break;
                case "nodeselector":
...
?>
                    <tr id="pm_buttons">
                        <td colspan="2">
                            <button id="add_pmr">Add Key Mapper</button>
                            <button id="rem_pmr">Remove Key Mapper</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<div style="float: right; width: 300px;">
        ...
        <p id="issue_reporting">
            <span id="reportissues_tt" class="tooltip"></span>Report Issues:
            <input type="checkbox" name="sscrap_report_issue" value="true" <?php echo ($report_issue)?'checked="checked"':''?>>
        </p>
        <p id="randomize_is">
            <span id="randomize_is_tt" class="tooltip"></span>Randomize
            <input type="checkbox" name="sscrap_randomize_is" value="true" <?php echo($randomize['is'])?'checked="checked"':''?>>
        </p>
        <p>
            <span id="randomize_min_tt" class="tooltip"></span>Minimum Interval
            <input type="text" name="sscrap_randomize_min_int" value="<?php echo $randomize['min_int']?>" size="2">
        </p>
        <p>
